...
def analyze_prompt_for_followup(prompt: str) -> str | None:
    ...
    **Task:**
    - If a follow-up for a critical missing detail (like a date for a leave request) is essential, provide the single, short question.
    ...
    **Examples of Your Judgment:**
    ...
    """
    try:
        print("--- [AI Brain] Analyzing prompt for follow-up...")
        response = model.generate_content(analysis_prompt)
        result_text = response.text.strip()

        if result_text == "NO_FOLLOWUP_NEEDED":
            print("--- [AI Brain] Analysis complete. No follow-up needed.")
            return None
        else:
